...
// памяти было потрачено впустую для поддержки многих ненужных микросхем контроллера.
// Теперь у вас есть возможность удалить этот ненужный код из библиотеки с помощью
// этот файл.
// Отключив ненужные контроллеры, вы можете уменьшить объем памяти
// библиотеки на несколько Кб.
//
// Раскомментируйте строки для контроллеров дисплея, которые вы не используете для сохранения
// некоторая флэш-память, не включая код инициализации для этой конкретной
// контроллер.
// ------------------
...
//#define DISABLE_ST7796			1	// ST7796