...
{   0x41, 0x80, 0x42, 0x81, 0x82, 0x45, 0x84, 0x85, 0x86, 0x87,
	0x4B, 0x88, 0x4D, 0x48, 0x4F, 0x89, 0x50, 0x43, 0x54, 0x8A,
	0x8B, 0x58, 0x8C, 0x8D, 0x8E, 0x8F, 0x90, 0x91, 0x92, 0x93,
	0x94, 0x95, 0x61, 0x96, 0x97, 0x98, 0x99, 0x65, 0x9B, 0x9C,
...
uint16_t UTFT::get_chars_by_printrus(String st, char* st_out)
{
	char buf[st.length() + 1];

	st.toCharArray(buf, st.length() + 1);
	return get_chars_by_printrus(buf, st_out);

}

uint16_t UTFT::get_chars_by_printrus(char* st, char* st_out)
{
	uint16_t utf_8_code, pos;

	pos = 0;

	for (uint16_t i = 0; i < strlen(st); i++)
	{
		if ((byte)st[i] < 0x7F)
		{
			st_out[pos] = st[i];
		...
		if ((byte)st[i] >= 0x7F)
		{
			utf_8_code += (byte)st[i] & 0x3F;
			if (utf_8_code == 0x0401) 
			{ 
				st_out[pos] = (char)code_byte_lcd[64];
				pos++;
			}

			else if (utf_8_code == 0x0451) 
			{ 
				st_out[pos] = (char)code_byte_lcd[65];
				pos++;
			}
			else {
				utf_8_code -= 0x0410;
				if (utf_8_code < 65) 
				{
					st_out[pos] = (char)code_byte_lcd[utf_8_code];
					pos++;
				}
			}
			utf_8_code = 0;
...
void UTFT::printrus(char* st, int x, int y, int deg)
{
	...
	char newst[strlen(st) + 1];
	get_chars_by_printrus(st, newst);
